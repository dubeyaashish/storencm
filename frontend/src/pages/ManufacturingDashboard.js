// client/src/pages/ManufacturingDashboard.js
...
const statusColors = {
  ...
  'Accepted by Manufacture': 'success',
  'Completed': 'success',
  'Rejected': 'error'
};
...
export default function ManufacturingDashboard() {
  ...
  useEffect(() => {
    ...
    const waiting = docs.filter(d => d.status === 'Send to Manufacture').length;
    const accepted = docs.filter(d => d.status === 'Accepted by Manufacture').length;
  ...
  useEffect(() => {
    let fd = [...docs];
    // Only show documents sent to manufacture
    fd = fd.filter(d => 
      d.status === 'Send to Manufacture' || 
      d.status === 'Accepted by Manufacture' || 
      d.status === 'Completed'
    );
    
    if (tabValue === 1) {
      fd = fd.filter(d => d.status === 'Send to Manufacture');
    } else if (tabValue === 2) {
      fd = fd.filter(d => d.status === 'Accepted by Manufacture');
    } else if (tabValue === 3) {
  ...
  // Handle Manufacturing accept
  const handleAccept = async (doc) => {
    if (!window.confirm('Accept this document for manufacturing?')) return;
    try {
      // The endpoint would need to be created on the backend
      await axios.post(`/api/documents/${doc.id}/accept-manufacturing`);
      ...
      setDocs(docs.map(d => {
        ...
        return {
          ...
          ManufacturingName: localStorage.getItem('userName') || 'Manufacturing',
          ManufacturingTimeStamp: new Date().toISOString()
        };
      }));
      
      setNotification({ 
        open: true, 
        message: 'Document accepted for manufacturing', 
        severity: 'success' 
      });
    } catch (err) {
      console.error('Error accepting for manufacturing:', err);
      setNotification({ 
        open: true, 
  ...
  if (loading) {
    return (
      <Box sx={{ p:3, textAlign:'center' }}>
        <Typography variant="h4">Manufacturing Dashboard</Typography>
  ...
  return (
    ...
    <Box sx={{ p:3 }}>
      ...
      {!error && !filteredDocs.length && (
        <Alert severity="info">
          No documents found. Manufacturing dashboards will only show documents that have been sent to manufacturing.
        </Alert>
      )}
              ...
              >
                <CardContent sx={{ flexGrow:1 }}>
                  ...
                  <Box display="flex" alignItems="center" mt={1}>
                    <ManufactureIcon fontSize="small" sx={{ mr: 1, color: 'text.secondary' }} />
                    <Typography variant="body2" gutterBottom>
