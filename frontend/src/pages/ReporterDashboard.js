// client/src/pages/ReporterDashboard.js
...
const ReporterDashboard = () => {
  ...
  useEffect(() => {
    ...
    const inProgress = docs.filter(d => 
      d.status.includes('Accepted') || 
      d.status.includes('Send')
    ).length;
    const completed = docs.filter(d => d.status === 'Completed').length;
    
    setStats({ total, created, inProgress, completed });
  }, [docs]);
    ...
    <>
      ...
      <Box sx={{ p: 3 }}>
        ...
        <Box 
          ...
          mb={3}
        >
          <Typography variant="h5">Reporter Dashboard</Typography>
        ...
        <Grid container spacing={2} mb={3}>
          {[
            ['Total', stats.total, '#2196f3'],
            ['Created', stats.created, '#ff9800'],
            ['In Progress', stats.inProgress, '#9c27b0'],
            ['Completed', stats.completed, '#4caf50']
